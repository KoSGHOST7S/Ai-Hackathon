...
const STEP_COMPLETE_AT = [30, 58, 84, 108, 130];
  ...
  return (
    <div
      style={{
        ...
        gap: 10,
        opacity: Math.max(
          interpolate(rowP, [0, 1], [0, 1]),
          isDone ? 1 : isActive ? 1 : 0.3
        ),
    ...
    >
      ...
      >
        {isDone ? (
          <div style={{ transform: `scale(${checkP})` }}>
            <svg
              width="16"
              height="16"
              viewBox="0 0 24 24"
...
const AIContent: React.FC<{ frame: number; fps: number }> = ({
  frame,
  fps,
}) => {
  const spinnerP = spring({
    frame,
    fps,
    config: { damping: 200 },
    durationInFrames: 22,
  });
  const allDone = frame > STEP_COMPLETE_AT[4] + 16;
            ...
            <div
              style={{
                ...
                transform: `scale(${spring({
                  frame: frame - (STEP_COMPLETE_AT[4] + 16),
                  fps,
                  config: { damping: 10, stiffness: 200 },
                  durationInFrames: 22,
                })})`,
        ...
        {allDone && (
          <div
            style={{
              ...
              opacity: interpolate(
                spring({
                  frame: frame - (STEP_COMPLETE_AT[4] + 20),
                  fps,
                  config: { damping: 200 },
                  durationInFrames: 20,
                }),
...
export const AIProcessingScene: React.FC = () => {
